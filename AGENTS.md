...
Este repositorio es parte de la colección camino troncal / Semestre 5 de
Jeresoft Academy y se rige por la RFC-0001 (manual fundacional).

## Objetivo

Crear el mejor recurso educativo posible sobre arquitectura de software en
Rust.
...
- El cuerpo del PR debe incluir una palabra clave de cierre reconocida por
  GitHub, por ejemplo `Closes #N`, además del resumen en español.
...
- Capítulo o fase: por ejemplo `capítulo: monolito-modular` o
  `flujo: issue-pr`.
...
Este curso no reemplaza a `rust-system-design`, `rust-distributed-systems`,
`rust-database-internals`, `rust-design-patterns`, `rust-cloud` ni
`rust-travel`. Esos cursos explican canónicos específicos. Este repositorio
organiza esas piezas dentro de límites, dependencias, contratos y evolución de
software.
...
- Presentar microservicios como madurez automática.
- Repetir teoría arquitectónica sin mostrar el costo en código.
